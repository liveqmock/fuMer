 ...
 *
 */
public class HttpSInComeForReqProcess implements HttpReqProcessInterface{
	private static Logger logger = LoggerFactory.getLogger(HttpSInComeForReqProcess.class);
	@Override
	public String httpProcess(Object o,String mchnt,String busiCd) {
		...
		try {
			...
			if(StringUtils.isEmpty(accntno)){
			...
			if(StringUtils.isEmpty(accntnm)){
			...
			if(MemcacheUtil.isForceVerifyMobile(mchnt)&&!RegexCheckUtil.checkMobile(mobile)){
				inComeForRspType.setRet(TDataDictConst.HTTP_MOBILE_ER);
				inComeForRspType.setMemo("请填写正确的手机号");
				rspXml = InComeForRspCoder.marshal(inComeForRspType);
				logger.error("return xml[" + rspXml + "]");
				return rspXml;
			}
			...
			InComeForBusiBean packBusiBean = (InComeForBusiBean) packagingBusiBean.toPackaging(operationVerifyErrorBusibean, calculateForTxnFeeAmtBean);
			OperationBean operationBean = packBusiBean.getInComeForFileBusiDetailRowBeanList().get(0).getOperationBean();
			if(operationBean.getHasError() || operationBean.isFeeHasError() || operationBean.isFeeSetD4HasError() || operationBean.isFeeSetHasError()){
				inComeForRspType.setRet(TDataDictConst.BUSI_ERROR);
				inComeForRspType.setMemo("手续费计算出错");
				rspXml = InComeForRspCoder.marshal(inComeForRspType);
				logger.error("return xml[" + rspXml + "]");
				return rspXml;
			}
			...
			if(fileErrorFromDbService.getFileSumError() != null){//汇总错误
			...
			if(fileErrorFromDbService.getFileRowErrors()!=null&&fileErrorFromDbService.getFileRowErrors().size()>0){//明细错误
