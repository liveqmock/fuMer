...
public class ExcelXLSXAnalysis extends ExcelAnalysis{
	private static Logger logger = LoggerFactory.getLogger(ExcelXLSXAnalysis.class);
	public TxnInfBean analysisTxn(String mchntCd, AccessBean accessBean, String txnInfSource, String txnDataType){
		...
		if(null != convertFile){
			...
			try {
				// 转换poi对象
				XSSFWorkbook book = new XSSFWorkbook(new FileInputStream(convertFile));
				...
				if(null != firstSheet){
					...
					if(sumRows<=0){
						...
						int rowSumFor = 0;
						XSSFRow row0 = firstSheet.getRow(0);
						try {
