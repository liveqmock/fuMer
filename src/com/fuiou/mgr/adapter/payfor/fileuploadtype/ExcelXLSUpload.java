 ...
 * 
 */
public class ExcelXLSUpload implements IFileUploadType {
	private static Logger logger = LoggerFactory
			.getLogger(ExcelXLSUpload.class);
	String mchntCd="";//商户号
	...
	public List<String> analysisTxn(FileAccess fileAccess) {
		...
		try {
			...
			if (firstSheet != null) {
				...
				if(TDataDictConst.BUSI_CD_PAYFOR.equals(fileAccess.getBusiCd())){//付款
					double sum=0;
					...
					for (int i = 1; i <= sumRows; i++) {
						...
						String bankDetailName = Utils.getCellValue(row.getCell(3));// 开户行支行全称
						String account = Utils.getCellValue(row.getCell(4));//收款人银行账号
						String accountName = Utils.getCellValue(row.getCell(5));// 户名
						...
						sum+=tempMoney;
						String serialNum = Utils.getCellValue(row.getCell(7));// 企业流水账号
						String remark = Utils.getCellValue(row.getCell(8));// 备注
						String mobilePhone = Utils.getCellValue(row.getCell(9));// 手机
