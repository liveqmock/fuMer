...
public class UploadFileAction {
	...
	private List<TFileErrInf> oneTxnErrListMap = new ArrayList<TFileErrInf>();
	/** 操作类型*/
	...
	public String executePayForBtnType() throws Exception {
		...
		if ("NO".equals(btnType)) {// 取消，删除文件信息与记录
			//删除文件
			if(tFileInfService.deleteTFileInfAndTApsTxnLogByWait(tOperatorInf.getMCHNT_CD(),fileInf.getFILE_NM(),busiCd) > 0){
	...
	public String executePayForUpload() throws Exception {
		...
		if(fileRowErrorList != null){
			...
			return "payfor_error";//错误提示页面，不允许提交交易
		}
		...
		if(payForBusiDetailRowBeanList.size()>0){
			try {
				...
				for(PayForBusiDetailRowBean payForBusiDetailRowBean:payForBusiDetailRowBeanListFee){
					//只有原金额-手续费>0，即目标金额>0的时候才计算
					if(payForBusiDetailRowBean.getDetailAmt()-payForBusiDetailRowBean.getOperationBean().getTxnFeeAmt() > 0){
						if(mchntTp.startsWith("D6") && payForBusiDetailRowBean.getOperationBean().getTxnFeeAmtId() == null){	// 月结商户计算手续费
							continue;
						}
				...
				tradeCalculate.checkAccountBalance(packBusiBean, fileError,srcModuleCd);//检查账户余额
				...
				for(PayForBusiDetailRowBean payForBusiDetailRowBean:payForBusiDetailRowBeanList){
					String feeMd =  payForBusiDetailRowBean.getOperationBean().getFeeMd();
					if("1".equals(feeMd)){
						sumAmt += payForBusiDetailRowBean.getDetailAmt();
					}else if("0".equals(feeMd)){
						sumAmt += payForBusiDetailRowBean.getDetailAmt()-payForBusiDetailRowBean.getOperationBean().getTxnFeeAmt();
					}else{
						sumAmt += payForBusiDetailRowBean.getDetailAmt();
					}
				}
				...
				if(fileRowOperateErrors.size() == 0){
					if(fileError.getFileSumError() != null){
						if(fileError.getFileSumError().getErrCode() != null||fileError.getFileSumError().getErrMemo() != null || fileError.getFileSumError().getRow() != null){
							fileRowOperateErrors = fileError.getFileRowErrors();
						}
					}
				}
				...
				if(oneTxnFileRowErrors!=null && oneTxnFileRowErrors.size() >= 0){
					boolean flag = false;//是否有错
