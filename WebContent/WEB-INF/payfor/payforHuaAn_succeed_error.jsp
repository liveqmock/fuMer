...
<s:if test="#oneTxnErrListMap.size > 0">
	收款错误描述：<br/>
	<s:iterator value="oneTxnErrListMap" var="oneTFeleErrorInf">
		<s:property value="ERR_ROW_DESC " /> <br/>
	</s:iterator>
</s:if>
<s:if test="#errListMap.size() > 0 ">
	<table  class="tableBorder" width="1098" border="0">
		...
		<tr>
			...
			<td width="122">扣款人开户行城市代码</td>
			<td width="123">收款人开户行支行名称</td>
		...
		<s:iterator value="errListMap" var="tFeleErrorInf">
			<tr>
				...
				<!--<td><s:property value="ERR_ROW_SEQ " /></td>
				<td><s:property value="FILE_NM " /></td>
...
<s:if test="#fileRows > 0">
	...
	url="${pageContext.request.contextPath}/payfor/upLoadTemp_executePayForBtnType.action?btnType=OK&uploadFileName=${fileNam}&saveFileExcel=${saveFileExcel }" />
&nbsp;&nbsp;<input type="button" value="取消" onClick="ver_del(this)" Class="type_button"
	url="${pageContext.request.contextPath}/payfor/upLoadTemp_executePayForBtnType.action?btnType=NO&uploadFileName=${fileNam}&saveFileExcel=${saveFileExcel }" />
</s:if>
 </div>
</body>
</html>